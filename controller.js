...
class Controller {
    ...
    async createUsluga (req, res) {
        ...
        const usluga = await db.query('insert into usluga (zagolovok, opisanie, zayavka_id) values ($1, $2, $3)', [zagolovok, opisanie, zayavka_id]);
        res.json(usluga.rows[0]);
    }
    async getUsluga (req, res) {
        const usluga = await db.query('select * from usluga');
        res.json(usluga.rows);
    }
    async updateUsluga (req, res) {
        const {id, zagolovok, opisanie, zayavka_id} = req.body;
        const usluga = await db.query('update usluga set zagolovok = $2, opisanie = $3, zayavka_id = $4 where id = $1', [id, zagolovok, opisanie, zayavka_id]);
        res.json(usluga.rows[0]);
    }
    async deleteUsluga (req, res) {
        const id = req.params.id;
        const usluga = await db.query('delete from usluga where id = $1', [id])
        res.json(usluga.rows[0]);
    }
}
